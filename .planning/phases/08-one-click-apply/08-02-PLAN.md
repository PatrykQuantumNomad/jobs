...
NOTE: This task has broad scope (8 methods, 100+ lines) but is a single cohesive class in one file. Splitting would create unnecessary cross-task coordination.
...
1. `async def apply(self, job: dict, mode: str, queue: asyncio.Queue) -> None`:
   ...
   - **Thread-safe emitter setup:** Before calling `asyncio.to_thread()`, capture the running event loop: `loop = asyncio.get_running_loop()`. Pass it to `_make_emitter(queue, loop)`.
   - Call `await asyncio.to_thread(self._apply_sync, job, mode, self._make_emitter(queue, loop))`
   - In finally block: remove from _sessions and _confirmations, emit DONE event

2. `def _make_emitter(self, queue: asyncio.Queue, loop: asyncio.AbstractEventLoop) -> Callable`:
   - Return a function that safely emits events from the background thread to the async queue:
     ```python
     def emit(event: ApplyEvent):
         loop.call_soon_threadsafe(queue.put_nowait, event.model_dump())
     return emit
     ```
   - `loop.call_soon_threadsafe()` is the correct way to interact with asyncio from a non-async thread. It schedules `queue.put_nowait(event_dict)` on the event loop thread. Do NOT call `queue.put_nowait()` directly from the background thread -- that is NOT thread-safe.
...
3. `def _apply_sync(self, job: dict, mode: str, emit: Callable) -> None`:
   ...
   - If browser platform (indeed, dice):
     ...
     - If confirm_before_submit: emit AWAITING_CONFIRM event with job details, then call `platform.wait_for_confirmation("Ready to submit?", timeout=300)` -- this uses the threading.Event internally when _dashboard_mode is True (see Task 2 below). Returns True if confirmed, False if timeout.
     - If confirmed: call `platform.apply(job_model, resume_path)`, emit DONE success
     - If timeout/not confirmed: emit ERROR "Confirmation timed out"
...
4. `def _fill_external_form(self, job: dict, mode: str, emit: Callable) -> None`:
   ...
   - If confirm_before_submit: emit AWAITING_CONFIRM, wait for confirmation via `self._confirmations[job["dedup_key"]].wait(timeout=300)` (external forms don't have a platform mixin, so use the threading.Event directly)
...
`python -c "from apply_engine.engine import ApplyEngine; e = ApplyEngine(); print('ApplyEngine created'); print(f'Sessions: {e._sessions}'); print(f'Has confirm: {hasattr(e, \"confirm\")}')"` succeeds with no import errors
  ...
  <done>
ApplyEngine class exists with apply(), confirm(), cancel(), get_session_queue() public API. Internal _apply_sync runs Playwright in thread. Events emitted via loop.call_soon_threadsafe(queue.put_nowait, ...) for thread safety. Semaphore enforces single concurrent apply. Resume path resolution checks tailored versions first. Activity timeline logging on start/complete/fail. Browser platform applies use platform.wait_for_confirmation() which internally uses threading.Event in dashboard mode.
  </done>
</task>
...
- _apply_sync runs in a thread and emits ApplyEvent objects to a queue via loop.call_soon_threadsafe()
...
- _apply_sync calls platform.wait_for_confirmation() for browser platforms (not direct threading.Event.wait())
